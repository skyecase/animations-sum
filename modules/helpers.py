...
def fade_and_shift_in(mobject: manim.VMobject, shift: np.array = manim.ORIGIN, fade_rate_func = manim.linear, shift_rate_func = cubic_out, scale = 1, **kwargs):
    ...
    def update_func(mobject: manim.VMobject, alpha: float):
        ...
        for i in range(len(leaf_mobjects)):
            leaf_mobjects[i].set_stroke(opacity = target_stroke_opacities[i] * fade_rate_func(alpha))
            leaf_mobjects[i].set_fill(opacity = target_fill_opacities[i] * fade_rate_func(alpha))
...
def fade_and_shift_out_color(mobject: manim.VMobject, shift: np.array = manim.ORIGIN, fade_rate_func = manim.linear, shift_rate_func = cubic_in, **kwargs):
    ...
    leaf_mobjects = get_all_mobjects(mobject)
    target_colors = [mobj.get_color() for mobj in leaf_mobjects]

    def update_func(mobject: manim.VMobject, alpha: float):
        mobject.move_to(target_pos + shift * shift_rate_func(alpha))
        for i in range(len(leaf_mobjects)):
            leaf_mobjects[i].set_color(manim.BLACK * fade_rate_func(alpha) + target_colors[i] * (1-fade_rate_func(alpha)))
    return manim.UpdateFromAlphaFunc(mobject, update_func, rate_func = manim.linear, remover=True, **kwargs)
